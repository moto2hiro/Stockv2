...
namespace Stock.Services.Models.CsvHelper
{
  public class MapInvestingWorldPrice : ClassMap<WorldPrice>
  {
    public MapInvestingWorldPrice()
    {
      ...
      Map(row => row.HighPrice).ConvertUsing(row => StringUtils.ToDecimal(row.GetField("High").Replace(",", "")));
      Map(row => row.LowPrice).ConvertUsing(row => StringUtils.ToDecimal(row.GetField("Low").Replace(",", "")));
